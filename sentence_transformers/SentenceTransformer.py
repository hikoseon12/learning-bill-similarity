...
class SentenceTransformer(nn.Sequential):
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, loss=None, save_result=True):
        """Runs evaluation during the training"""
        if evaluator is not None:
            score, acc, test_classification_report, test_confusion_matrix = evaluator(
                self, output_path=output_path, epoch=epoch, steps=steps, loss=loss, save_result=save_result)
            if score > self.best_score and save_best_model:
                self.save(output_path)
                print("{} is better than {}. so it is saved".format(
